...
export async function sign(alg: string, key: CryptoKey | Uint8Array, data: Uint8Array) {
...
export async function getSignVerifyKey(alg: string, key: CryptoKey | Uint8Array, usage: KeyUsage) {
  if (key instanceof Uint8Array) {
    if (!alg.startsWith('HS')) {
      throw new TypeError(`Key must be ${key} of type: CryptoKey or JSON Web Key`);
    }
...
export function subtleAlgorithm(alg: string, algorithm: KeyAlgorithm | EcKeyAlgorithm) {
  ...
  switch (alg) {
    ...
    case 'PS384':
    case 'PS512': {
      return { hash, name: 'RSA-PSS', saltLength: Number.parseInt(alg.slice(-3), 10) >> 3 }
    }
    case 'RS256':
    case 'RS384':
    case 'RS512': {
