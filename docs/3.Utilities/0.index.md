...
Decodes a Base64URL string. Returns a `string` by default, or a `Uint8Array` if `toString === false`.

```ts
base64UrlDecode("aGVsbG8"); // "hello"
base64UrlDecode("aGVsbG8", false); // Uint8Array([104, 101, 108, 108, 111])
```

### `base64Encode(data)` / `base64Decode(str?, toString?)`

Standard Base64 (with `=` padding). Same signatures as the URL-safe variants. Use when interoperating with systems that expect classical Base64.
...
const cek = randomBytes(32); // 256-bit CEK
```
...
```ts
type Duration = number | `${number}${Unit}` | `${number} ${Unit}`;
