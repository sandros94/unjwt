...
hooks: {
  onVerifyKeyLookup({ header, event, config }) {
    // Return any JWK, JWKSet, JWK_Symmetric, or JWK_Public
...
`SessionHooksJWE` has the same shape; swap JWS types for JWE types, and `onVerifyKeyLookup` for `onUnsealKeyLookup` (returns a symmetric or private JWK).
