...
interface SessionConfigJWE<T, MaxAge, TEvent> {
  key:
    ...
    | { privateKey: JWK_Private | JWK_Symmetric; publicKey?: JWK_Public }; // asymmetric
...
// Updater function
await session.update((old) => ({ count: (old.count ?? 0) + 1 }));
...
- JWE: `httpOnly: true, secure: true, sameSite: "lax"`.
- JWS: `httpOnly: false, secure: true, sameSite: "lax"` (the payload is meant to be readable).
