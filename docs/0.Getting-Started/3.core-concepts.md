...
## Keys: representations and secrecy

Two ideas trip up newcomers more than anything else in the Web Crypto world. Both are worth getting straight early.

### A key has two representations

| Representation | What it is                               | Where the key material lives                                                        |
| -------------- | ---------------------------------------- | ----------------------------------------------------------------------------------- |
| **JWK**        | A plain JSON object (`{ kty, k, ... }`)  | **In the object** — `k` (symmetric) or `d` (private) _is_ the secret, in plain text |
| **CryptoKey**  | An opaque handle from the Web Crypto API | **Inside the runtime** — your code never sees the bytes                             |

A JWK is portable and inspectable: easy to store, log, or send — which also means easy to leak. A `CryptoKey` is a reference the runtime signs and decrypts _on your behalf_; the raw bytes stay locked away. unjwt accepts both everywhere and normalizes internally, so you choose based on what you need: JWKs for portability, `CryptoKey`s for keeping material out of reach.

### Public, private, and secret are three things, not two

```
asymmetric pair                        symmetric key
┌─────────────────────┐               ┌───────────────┐
│  public  (n,e/x,y)  │  ← shareable  │   secret (k)  │  ← NEVER shareable
│  private (d)        │  ← secret     │               │
└─────────────────────┘               └───────────────┘
```

- A **public** key (asymmetric, no `d`) is meant to be published — it only verifies or encrypts.
- A **private** key (asymmetric, has `d`) must stay secret — it signs or decrypts.
- A **secret** key (symmetric, `kty: "oct"`, has `k`) is the whole secret on its own — used for `HS*` signing and AES — and is **never** safe to share.

The catch: "secret" keys are not "private" keys in the `d` sense. So `isPrivateJWK(key)` returns `false` for a symmetric key — and `!isPrivateJWK(key)` is therefore **not** a "safe to publish" test, because it passes every `oct` key. To filter keys you can safely hand out, use `isPublicJWK(key)` (true only for asymmetric public keys); treat anything `isSymmetricJWK(key)` reports as a secret. → [Type guards](/utilities#type-guards)

### Extractability protects long-lived keys

When you import or generate a `CryptoKey`, an `extractable` flag decides whether its raw bytes can ever be read back out (via export to JWK/PEM). With `extractable: false`, the runtime will still sign and decrypt with the key, but its material can never be exfiltrated — even by code running in the same process.

For long-lived asymmetric private keys that should never leave the server, prefer a non-extractable `CryptoKey`. Note that producing a JWK inherently requires extractable material (the bytes have to be readable to serialize them), so the two goals are mutually exclusive by design. → [Importing & exporting](/jwk/import-export)
