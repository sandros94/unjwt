...
| `wrappingKey` | `CryptoKey \| JWK \| string \| Uint8Array`   | The recipient's key (or password for PBES2).      |
...
const cek = await unwrapKey("ECDH-ES+A256KW", encryptedKey, myPrivateKey, {
  epk: tokenHeader.epk,
  apu: base64UrlDecode(tokenHeader.apu, false),
  apv: base64UrlDecode(tokenHeader.apv, false),
