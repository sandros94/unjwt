...
| `key`                     | `JWK \| JWKSet \| CryptoKey \| Uint8Array \| JWKLookupFunction`                                                               |
...
It can return any of: `JWK`, `JWKSet`, `CryptoKey`, `Uint8Array`, or a `string`. Async is allowed.
...
When inference is impossible, unjwt silently accepts any signature algorithm matching the key type, which is **not what you want** in security-sensitive contexts. Inference returns `undefined` for:

- Raw `Uint8Array` keys.
- JWKs without an `alg` field.
- Lookup functions.
...
- **`undefined`** (default) — validate whenever the decoded payload is an object.
- **`true`** — error if the payload isn't an object (strict JWT mode).
